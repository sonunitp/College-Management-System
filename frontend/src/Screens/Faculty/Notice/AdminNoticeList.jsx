...
const AdminNoticeList = () => {
  ...
  const fetchNotices = () => {
    setLoading(true);
    axios
      .get(`${baseApiURL()}/notice/getAllNotices/111111111111111111111111/${facultyId}`)
      .then((response) => {
          ...
          {notices.map((notice) => (
            <div key={notice._id} className="bg-blue-50 p-6 rounded-lg shadow-md relative">
              ...
              <p className="text-sm text-gray-600 mt-2">
                Issued by: {`${notice.issuedByFaculty?.firstName } ${notice.issuedByFaculty?.lastName }`|| "Unknown"} |{" "}
